...
def count_best_combinations(array):
    count = 0

    for first_product in range(len(array) - 1):
        for second_product in range(first_product + 1, len(array)):
            if array[first_product] == array[second_product]:
                count += 1
